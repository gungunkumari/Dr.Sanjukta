...
//  All interactive features + Backend form submission
// ============================================================
...
backToTopBtn.style.cssText = `
  ...
  background: #2E8B57; color: #fff; font-size: 20px;
...
toastStyle.textContent = `
  ...
  .nav-links a.active-link { color: #2E8B57 !important; font-weight: 600; }
  .form-error { display: block; color: #e53e3e; font-size: 12px; margin-top: 5px; min-height: 16px; }
  .form-fail  { display: none; background: #fff5f5; color: #c53030; border: 1.5px solid #feb2b2; border-radius: 12px; padding: 14px 18px; font-size: 14px; font-weight: 600; text-align: center; margin-top: 12px; }
  #fname.invalid, #fphone.invalid, #femail.invalid { border-color: #e53e3e !important; box-shadow: 0 0 0 3px rgba(229,62,62,.12) !important; }
`;
document.head.appendChild(toastStyle);


// ── 9. BOOKING FORM — CONNECTED TO BACKEND ─────────────────
// ✅ This sends data to Node.js backend → saves to appointments.json
// ✅ Doctor can see it in portal at http://localhost:3000/portal.html
...
async function submitForm() {
  ...
  if (!name) {
    ...
    if (e) e.textContent = 'Please enter your full name';
    valid = false;
  }
  if (!phone || phone.replace(/\D/g,'').length < 7) {
    document.getElementById('fphone').classList.add('invalid');
    const e = document.getElementById('err-phone');
    if (e) e.textContent = 'Please enter a valid phone number';
    valid = false;
  }
  ...
  try {
    ...
    // Same backend for both local + production
    const backendUrl = '/appointment';
    ...
    if (res.ok && result.success) {
      ...
      // ❌ Server error
      if (fail) {
        fail.textContent   = '❌ ' + (result.error || 'Something went wrong. Please try again.');
    ...
    if (fail) {
      fail.textContent   = '❌ Cannot connect to server! Make sure backend is running: node server.js';
      fail.style.display = 'block';
    }
    btn.textContent = 'Send Request ✈️';
    btn.disabled    = false;
